...
class ReceptiveFieldBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):    # count filters = out_channels --> every filter learns to detect features in every in_channel
        ...
        self.conv02 = nn.Conv2d(2*inter_channels, branch_out_channels, kernel_size=3, padding=1)  # 3x3 kernel, padding=1 for same output size
        ...
        self.conv13 = nn.Conv2d(2*inter_channels, branch_out_channels, kernel_size=3, dilation=3, padding=3) # padding=dilation for same output size
        ...
        self.conv23 = nn.Conv2d(2*inter_channels, branch_out_channels, kernel_size=3, dilation=5, padding=5) # padding=dilation for same output size
        ...
        self.conv32 = nn.Conv2d(4*inter_channels, 6*inter_channels, kernel_size=(1,7), padding=(0,3)) # padding=(0, (7-1)//2) for same output size
        self.bn32 = nn.BatchNorm2d(6*inter_channels)

        self.conv33 = nn.Conv2d(6*inter_channels, 6*inter_channels, kernel_size=(7,1), padding=(3,0)) # padding=((7-1)//2, 0) for same output size
        self.bn33 = nn.BatchNorm2d(6*inter_channels)
        
        self.conv34 = nn.Conv2d(6*inter_channels, branch_out_channels, kernel_size=3, dilation=7, padding=7) # padding=dilation for same output size
    ...
    def forward(self, x_in):
        ...
        x = torch.cat((x0, x1, x2, x3), 1)  # concatenate along the channel dimension (add in channel dimension: x.channels = 4* branch_out_channels)
